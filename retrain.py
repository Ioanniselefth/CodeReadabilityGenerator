...
if os.path.exists('feedback.csv'):
    ...
    feedback_df['correct'] = feedback_df['correct'].apply(lambda x: 1 if x == 'yes' else 0)
    feedback_df.rename(columns={'snippet': 'python_solutions', 'correct': 'readability'}, inplace=True)
    df = pd.concat([df, feedback_df], ignore_index=True)

# Handle missing values by filling with an empty string
df['python_solutions'] = df['python_solutions'].fillna('')
...
X_text = vectorizer.fit_transform(df['cleaned_snippet']).toarray()
X_additional = df[['num_lines', 'avg_line_length', 'num_comments', 'indentation_consistency', 'cyclomatic_complexity', 'unbalanced_brackets', 'contains_digits', 'avg_word_length', 'num_functions']].values
